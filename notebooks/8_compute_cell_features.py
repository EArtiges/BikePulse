...
def get_features(cell_index, nth_neighbours="first_neighbours"):
    ...
    features |= (
        cycleways[cycleways.intersects(cell)].groupby("highway")["real_length"].sum().to_dict()
    )
...
features = {cell_index: get_features(cell_index) for cell_index in hex_grid.index}
