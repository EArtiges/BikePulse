...
def get_processed_trips(trips, station_distances):
    ...
    trips = trips.join(
        station_distances.stack().rename("distance"), on=["start_station_id", "end_station_id"]
    )
...
def get_matrix(data, clusters, sample_size=None):
    ...
    if sample_size:
        ...
        data_sample = data
    matrix = (
        data.groupby(["hour", "start_cluster", "end_cluster"])
        .trip.sum()
        .unstack()
        .fillna(0)
        .astype(int)
    ...
    rev_matrix = (
        data.groupby(["hour", "end_cluster", "start_cluster"])
        .trip.sum()
        .unstack()
        .fillna(0)
        .astype(int)
...
def bootstrap_T(matrix, frac, data, clusters):
