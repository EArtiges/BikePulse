...
cell_distances = pd.DataFrame(
    index=hex_grid.index, columns=hex_grid.index, data=distance_matrix(cell_GPS, cell_GPS)
).astype(int)
...
station_distances = pd.DataFrame(
    index=stations.index, columns=stations.index, data=distance_matrix(stations_GPS, stations_GPS)
).astype(int)
...
trips = oslo_lib.get_processed_trips(
    pd.read_pickle(f"data/{city}/trips/trips.pkl"), station_distances
)
...
departures_per_station = (
    trips.groupby(["start_station_id", "year", "month", "weekday", "hour"])
    .trip.sum()
    .rename("departures_per_station")
)
arrivals_per_station = (
    trips.groupby(["end_station_id", "year", "month", "weekday", "hour"])
    .trip.sum()
    .rename("arrivals_per_station")
)
...
OD_matrix = trips.groupby(
    ["year", "month", "weekday", "quarter", "hour", "start_cell", "end_cell"]
)["trip"].sum()
